...
func (cache *inMemoryCache) NameByIP(ip []byte) (name string, found bool) {
	e, found := cache.namesByIP[IPToInt(ip)]
	if !found {
		return "", false
	}
...
func (cache *inMemoryCache) NextSequence() uint32 {
	...
	next := cache.sequence
	cache.sequence++
	if cache.sequence > MaxIP {
		// wrap IP to stay within allowed range
		cache.sequence = MinIP
	}
	return next
}

func (cache *inMemoryCache) Close() error { return nil }
